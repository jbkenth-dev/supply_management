...
function handleUpdate(PDO $pdo): void
{
    ...
    if (isset($_FILES['profileImage']) && is_array($_FILES['profileImage']) && (int) $_FILES['profileImage']['error'] !== UPLOAD_ERR_NO_FILE) {
        $profileImagePath = storeProfileImage($_FILES['profileImage'], (int) $id);
    }
    ...
    $updatedUser = findAdminUser($pdo, (int) $id);
...
function findAdminUser(PDO $pdo, int $id): ?array
{
    $query = $pdo->prepare(
...
function storeProfileImage(array $file, int $userId): string
{
    ...
    $filename = sprintf('admin-%d-%d.%s', $userId, time(), $extensions[$mimeType]);
